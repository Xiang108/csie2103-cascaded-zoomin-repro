...
def inference_with_crops(model, data_loader, evaluator, cfg, iter):
    ...
    # val 評估時移除 cluster 類別，避免 COCO mAP 維度不一致
    thing_classes = evaluator._metadata.get("thing_classes")
    if thing_classes and thing_classes[-1] == "cluster":
        evaluator._metadata.set(thing_classes=thing_classes[:-1])
